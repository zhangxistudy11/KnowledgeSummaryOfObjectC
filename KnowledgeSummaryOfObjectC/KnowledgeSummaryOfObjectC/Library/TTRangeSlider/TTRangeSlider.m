...
- (void)initialiseControl {
    ...
    CGFloat height = self.labelHeight? : TEXT_HEIGHT;
    self.minLabel.frame = CGRectMake(0, 0, 75, height);
...
- (void)updateLabelValues {
    ...
    if ([self.delegate respondsToSelector:@selector(setMixLabel:andMaxLabel:)]) {
        [self.delegate setMixLabel:self.minLabel andMaxLabel:self.maxLabel];
    } else {
...
- (void)updateLabelPositions {
    ...
    if (self.lineAndLabelSpace!=0) {
        padding = self.lineAndLabelSpace;
    }
